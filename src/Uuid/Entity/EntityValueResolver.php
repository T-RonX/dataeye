...
#[AsTaggedItem('controller.argument_value_resolver', priority: 150)]
final class EntityValueResolver implements ValueResolverInterface
{
    ...
    public function resolve(Request $request, ArgumentMetadata $argument): array
    {
        ...
        if (null === $object && (!$argument->isNullable() || $this->getIdentifier($request, $options, $argument->getName()) !== null)) {
    ...
    private function find(ObjectManager $manager, Request $request, MapEntity $options, string $name): false|object|null
    {
        ...
        try {
            if (is_subclass_of($options->class, EntityUuidInterface::class)) {
                return $manager->getRepository($options->class)->findOneBy(['uuid' => $id]);
            }
